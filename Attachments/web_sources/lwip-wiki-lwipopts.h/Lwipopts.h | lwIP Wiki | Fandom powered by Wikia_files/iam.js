...
var iom = iom || (function () {
  var dummySite = "dummy",
      ...
      eventList = ["", "inst", "init","open", "clse", "play", "resm", "stop", "fowa", "bakw", "recd", "paus", "forg", "bakg", "dele", "refr", "kill", "view", "alve", "fini", "aforg", "abakg", "aclse", "sple", "scvl", "serr", "spyr", "smdr", "sfpl", "sfqt", "ssqt", "stqt", "soqt", "sofc", "scfc", "scqt", "splr", "spli", "sprs", "spre", "smrs", "smre", "sors", "sore", "sack"],
      LSOBlacklist = [],
      checkEvents = 1,
      tb = 0,
      sv = 1,
  ...
  function event(event) {
    ...
    if (inited && !isDoNotTrack() && (!checkEvents || (checkEvents && arrayContains(eventList, event)))) {
      ...
      if (arrayContains(['play', 'resm', 'alve', 'sfqt', 'ssqt', 'stgt'], event) && mode === 1 && result.hasOwnProperty('hb')) {
        startHeart();
      }
      return result;
    }
  ...
  function fetchTizenUUIDs() {
    try {
      return '{"uuids":{"tizenid":"' + tizen.systeminfo.getCapability('http://tizen.org/system/tizenid') + '","duid":"' + webapis.productinfo.getDuid() + '"}}';
    } catch(e) {
      return 'notizen';
      //pass
    }
